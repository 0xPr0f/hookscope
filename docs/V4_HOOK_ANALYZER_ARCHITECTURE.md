...
- Vite/React/TypeScript application, capability-filtered chain selector, address validation before RPC access, responsive report UI, JSON export, cancellation, IndexedDB history, and completed-report-only persistence flow. The app-wide accessible dropdown is shared by the chain and contract-source selectors; its menu is scrollable and bounded by its owning control.
...
- Dedicated static worker using the upstream WhatsABI and EVMole packages. EVMole is explicitly initialized from its supported `no_tla` entrypoint and emitted as a separate Wasm asset.
...
- Typed fork-state hydration protocol: revm requests missing accounts, code, storage slots, or ancestor block hashes; the browser reads them at the pinned parent block and retries. Hydrated state is retained inside a Wasm fork session, while one scan-wide immutable cache reuses successful pinned reads across replay, scenario, and exploration sessions. Concurrent balance/nonce/code reads use bounded JSON-RPC batches. Independent observations discard transaction overlays; multi-step scenarios explicitly commit successful/reverted transaction results so nonce and state sequencing match chain execution. A historical receipt must match outcome, log count, and gas before replay can pass.
...
- Pool-wide historical replay orchestration prioritizes recent swaps, then liquidity, donation, and initialization references. It uses one replay worker by default, tries at most three indexed references until one receipt-matched context passes per pool, and stops downloading redundant contexts after success. Missing contexts or archive state produce explicit degraded coverage; a receipt mismatch cannot pass. Static work remains available when optional replay context is unavailable.
- Direct calls to deployed hook callbacks are not live PoolManager tests: common hooks reject non-PoolManager callers, and post-callbacks may depend on an active PoolManager unlock/transient context. Public reports therefore count only receipt-matched router executions whose trace reaches both the configured PoolManager and selected hook. A supported Universal Router/PositionManager envelope receives controlled variants; an unsupported custom router receives an exact unchanged historical execution observation, not invented mutations.
- Receipt-matched Universal Router and PositionManager payloads are decoded with a conservative official codec. It supports installed v1.0.3 and current v2 swap tuple layouts, rejects non-canonical calldata, matches full PoolKeys/paths to the selected PoolId, preserves unrelated commands, settlement actions, deadline, caller, router, value, and parent block, then replays isolated hookData and smaller-amount variants in a reusable pinned fork session. Direct PositionManager token-ID actions use a cached `getPoolAndPositionInfo` read at the pinned parent block and must reconstruct the selected PoolId. Canonical `EXECUTE_SUB_PLAN` branches follow the official [`0x21`/`0xa1` command](https://github.com/Uniswap/universal-router/blob/2.2.0/contracts/libraries/Commands.sol) and [`(bytes commands, bytes[] inputs)` dispatcher layout](https://github.com/Uniswap/universal-router/blob/2.2.0/contracts/base/Dispatcher.sol) through a four-level application recursion ceiling with stable nested locations. Signature-bearing payloads are now identified explicitly — Universal Router `PERMIT2_PERMIT_BATCH` (0x03), `PERMIT2_PERMIT` (0x0a), and `V3_POSITION_MANAGER_PERMIT` (0x11), plus the PositionManager `permit`/`permitForAll`/Permit2Forwarder `permit`/`permitBatch` entrypoints reachable inside `multicall`. They are replayed byte-identically, and mask derivation proves it by flipping every masked byte at once and requiring the signed payloads to survive unchanged, so no generated variant can invalidate an original signature. Reports state which signed standards were present and what their messages commit to. Universal Router token-ID attribution remains an explicit coverage gap.
- Coverage-guided mutation runs against hydrated live fork targets. One canonical receipt-matched router transaction per pool is paired with a mutation mask restricted to the operations that provably act on that pool, then explored in one hydrated revm session under a single 30,000-execution/30-second budget split across two rounds. Missing state reported by a round is hydrated before the next round starts; interesting inputs, including those skipped for that state, are handed forward as a compact corpus and re-checked against the mask so an exchanged seed can never carry a changed router envelope. Fan-out is across pools rather than inside a pool because hydrated fork state cannot be transferred between Wasm sessions. The phase is bounded to three targets per scan and every uncovered hooked pool is an explicit `unsupported` outcome.
...
- Production build, 145 Vitest checks across 35 files (including the complete 80-transaction Wasm suite, router-codec/variant coverage, fork-hydration caching, live replay selection, execution-client classification, pipeline-version cache invalidation, and a masked-router exploration gate that runs the real LibAFL/revm Wasm bridge over a canonical 1,060-byte Universal Router payload) and 11 tooling checks pass. Native revm, Foundry, and desktop-browser conformance remain separate release gates rather than being implied by the browser unit-test count.
...
- Canonical Universal Router v4 swaps, nested subplans, and PositionManager PoolKey-bearing operations now receive controlled variants using their historical funded/approved actor and settlement envelope. Direct PositionManager token-ID actions are resolved at the pinned parent block; token-ID actions routed through Universal Router remain unattributed because the outer calldata does not provide the immutable PositionManager address. Donation variants through a production router are **closed by design, not pending**: `Actions.DONATE` is `0x0a`, upstream marks it "not supported in the position manager or router", and both `V4Router._handleAction` and `PositionManager._handleAction` branch on `action < Actions.SETTLE (0x0b)` and fall through to `revert UnsupportedAction`. No canonical envelope exists to recognize. Donations reach a pool only through a custom contract calling `PoolManager.unlock` directly, which is covered by historical replay and by the Hacken suite's own donate router.
...
- Live fork-state input exploration now covers pool-attributed canonical router payloads. What remains is widening what a mask may own: donation and signed envelopes are not decoded, token-ID-only actions are excluded until their attribution is reproducible, and only primary amount and existing hookData bytes are mutable — dynamic lengths, PoolKeys, recipients, and deadlines are deliberately immutable.
...
Live-canary result: the local Ethereum subgraph proxy returns the supplied HFA token's pool, four recent swap references, two liquidity references, and a synced/indexing-error-free head in one request. Its prioritized recent swap reproduced outcome, three logs, and 119,343 gas in browser revm after 20 logical pinned-state hydration requests. The outer transaction used an unsupported custom-router selector, so its exact PoolManager/hook trace is reportable but controlled calldata mutation remains explicitly unavailable. Full-history browser RPC scanning remains a bounded fallback used only when both static index and subgraph fail.
...
- Server-side code is limited to completed-report storage and a narrow allowlisted pool-discovery proxy that keeps the Graph API key out of the browser; no analysis executes on the server.
...
React UI
  -> feature hook / scan controller
      ...
      -> analysis worker coordinator
          ...
          -> hydrated fork explorer (pool-scoped router mask + inter-round hydration)
...
4. For the selected batch only, query each initialization block by indexed PoolId when the discovery source did not include its transaction hash. The configured pool-entity subgraph also returns bounded recent swap and modify-liquidity transaction references in the same token-filtered query; a static index may additionally supply initialization and donation references.
...
7. Validate every replay reference against its transaction, receipt, event kind, configured PoolManager, and PoolId, then replay at the parent block. Run one replay worker by default, prioritize behavior-bearing references, try at most three fallbacks, and stop after the first receipt-matched context per pool.
8. Deduplicate static analysis by `{chainId, codehash}` while retaining pool-specific replay and scenario evidence.

Published Graph Network subgraph IDs live in the chain registry because they carry no credential material. The browser uses the same-origin `/api/subgraph/<chainId>` endpoint; Vite implements it locally and Vercel implements it in production, while `SUBGRAPH_API_KEY` remains server-only. The endpoint accepts only the chain's exact token-discovery query and bounded variables, so it is not a general GraphQL relay. A per-chain `VITE_V4_SUBGRAPH_<chainId>` may still override this for a credential-free private/self-hosted indexer. Subgraph results are candidates only — every PoolId is recomputed and its initialized state verified against the pinned chain snapshot, so a stale or wrong index can withhold pools but can never introduce one.
...
| EVMole | Ready | Sole bytecode engine: Wasm disassembly, selectors, argument types, mutability, storage records, basic blocks, and control-flow graph, with full Cancun opcode coverage. |
...
| LibAFL Wasm | Ready for bounded runtime-bytecode and hydrated-fork exploration | Pinned 0.15.4 is the newest release compatible with the project's Rust 1.88 toolchain. Each Dedicated Worker owns one upstream queue scheduler, havoc mutator, in-memory corpus, and revm edge map. The coordinator applies one shared budget, exchanges compact seeds for a second round, minimizes retained outcomes, and keeps cancellation/trap isolation at the Worker boundary. Fork targets mutate only masked calldata bytes; the mask is capped at the bridge's 512-byte input ceiling. |
...
2. **Bytecode pass:** WhatsABI proxy/interface inference plus EVMole decoding, CFG, and storage records.
...
| `FuzzTestEntry` | Deterministic boundary corpus plus separate 30,000-execution LibAFL/revm edge corpus | Two-round worker fan-out, compact seed exchange, and outcome minimization pass for runtime-bytecode and hydrated-fork targets |

The complete 40-scenario/80-transaction browser port is a deterministic conformance gate and runs only for the generated PoolManager fixture on a desktop runtime. Public-token reports do not pretend that fixture assertions ran against an arbitrary deployed hook. They execute the portable live subset only when the chain is certified for deep execution, its PoolManager is configured, at least one discovered non-zero hook has a receipt-matched historical transaction, and the actor/router/parent-block context can be hydrated. Mobile v1, uncertified chains, empty pool sets, unhooked pools, missing receipts/archive state, disabled hook permission bits, and unrecognized router envelopes remain explicit `SKIP`/degraded outcomes. Mobile classification uses an explicit browser mobile hint or a narrow touch/coarse-pointer client; a narrow desktop pane remains eligible for deep execution. The Tests tab formats these results like a Foundry run while preserving assertion `PASS`/`FAIL`, non-assertive `OBSERVED`, and `SKIP` as separate states.
...
#### Hydrated fork targets

A public-chain scan explores the deployed pool itself rather than a bytecode fixture. Each selected pool contributes one canonical receipt-matched router transaction, and its mutation mask is derived only from the operations that provably act on that PoolId. The mask exposes the low-order bytes of a swap's primary amount, the full value bytes of a liquidity scalar, and the contents of existing hookData. Dynamic lengths, PoolKeys, paths, recipients, deadlines, settlement commands, and every unrelated command stay byte-identical, and the mask is capped at the bridge's 512-byte input ceiling.

The session is hydrated once by replaying the historical transaction, then explored in two rounds under one shared 30,000-execution/30-second budget. A round returns the state it could not resolve; the coordinator fetches that state at the pinned parent block and starts the next round with it, seeding it with the previous round's minimized witnesses and the inputs that were skipped for exactly that missing state. Every exchanged seed is re-validated against the mask, so a candidate that changed the envelope cannot re-enter exploration. Worker fan-out is applied across pools, not inside one pool: hydrated fork state cannot be transferred between Wasm sessions, so a second session for the same pool would repeat its pinned reads rather than share them. The phase is bounded to three hydrated targets per scan, and any hooked pool left uncovered — no recognized payload, or beyond the ceiling — is reported as an explicit `unsupported` outcome rather than silently omitted.
...
| sevm | removed 2026-08-15 | Its opcode table ends at PUSH0, so `new Contract()` threw on every Cancun contract — meaning every Uniswap v4 hook using EIP-1153 transient storage. Upstream published 0.7.4 in December 2024 and last committed in March 2025. EVMole already supplies disassembly, basic blocks, and CFG, so the package contributed only a swallowed exception for 7.2 MB of installed weight. |
| heimdall-rs | evaluated and rejected 2026-08-15 | Not published to crates.io, and every analysis crate depends on `heimdall-common`, which pulls `tokio`, `reqwest`, `async-openai`, `indicatif`, and `crossbeam-channel`, plus a filesystem cache. Its `alloy` stack also requires rustc 1.91 against this project's 1.88 pin. A `wasm32-unknown-unknown` build on 1.91 fails at `mio` because heimdall enables tokio's `net` feature itself, which a downstream crate cannot disable. Adopting it would require forking a CLI-coupled decompiler, and a hand-stripped decompiler is not a defensible evidence source. |
...
The accepted concept is a 1440×1050 evidence-led desktop surface with a responsive single-column mobile continuation. Completed reports use four top-level tabs—Overview, Evidence, Contracts, and Tests—so detailed ledgers and source text do not create one unbounded page.

- Light mode uses a true-white background, cool gray rules, and near-black ink. Dark mode uses the same semantic design tokens and may follow the operating-system preference or the header toggle.
...
- Overview contains the phase rail, assessment, pool/callback/capability coverage, limitations, rerun/continuation actions, and history. Evidence contains the expandable ledger and export. Tests renders assertion results in a Foundry-style console while preserving `OBSERVED` as distinct from assertion `PASS`.
- Contracts lazily loads CodeMirror 6 plus the Replit Solidity grammar. One fixed-height workbench contains the token/hook selector, verified-file explorer, file/source search, declaration outline, open-file tab, line numbers, folding, keyboard search, copy action, and an independently scrolling read-only editor. The editor theme follows the site; the roughly 137 KB-gzip editor bundle is split from the initial application chunk.
