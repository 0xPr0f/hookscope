...
export type SelectorSignatureLookup = Readonly<Record<string, readonly SelectorSignatureCandidate[]>>
...
function normalizeSelector(value: string): Hex | undefined {
...
function normalizedCandidates(value: unknown): SelectorSignatureCandidate[] {
  ...
  const candidates = value.flatMap((candidate): SelectorSignatureCandidate[] => {
    ...
    return [{ name: item.name, hasVerifiedContract: item.hasVerifiedContract === true }]
  })
...
export async function fetchSourcify4ByteSignatures(
  ...
  const requested = [...new Set(
    [...selectors]
      .flatMap((selector) => normalizeSelector(selector) ?? [])
      .slice(0, MAX_SELECTORS),
  )]
  const missing = requested.filter((selector) => !candidateCache.has(selector))

  await Promise.all(chunks(missing, BATCH_SIZE).map(async (batch) => {
